...
export function SolarDiagram({ width, height, prefix, selectedBodyId = null, viewportWidth = width }) {
  const view = getOverview(width, height)
  const composition = selectedBodyId === CORE.id ? focusComposition(SUN, viewportWidth) : null
  const zoom = composition ? composition.heightFraction * view.tanY * view.distance / SUN.radius : 1
  const center = { x: (1 + (composition?.x || 0)) * width / 2, y: (1 - (composition?.y || 0)) * height / 2 }
  function project(point) {
    const p = projectOverview(point, view)
    return { x: center.x + p.x * width / 2 * zoom, y: center.y + p.y * height / 2 * zoom, depth: p.depth }
  }
  function path(orbit) {
  ...
  return <svg className="GalaxySolarDiagram" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={composition ? `${CORE.signal}: ${CORE.name}, the Sun at the center of the system` : `Solar system: ${CORE.shortName} at the center, Identity, Skills, Projects and Journey on four orbits, and a distant Lab signal`}>
    <defs>
      {bodies.map(body => {
        const dx = center.x - body.x, dy = center.y - body.y, length = Math.hypot(dx, dy) || 1
    ...
    {bodies.map(body => <g key={body.id} data-body={body.id} opacity={selectedBodyId && selectedBodyId !== body.id ? .7 : 1}>
      {selectedBodyId === body.id && body.id !== CORE.id && <circle cx={body.x} cy={body.y} r={body.r * (body.ring ? 2.4 : 1.85)} fill="none" stroke={body.color} strokeWidth=".8" strokeDasharray="2 5" opacity=".65" />}
